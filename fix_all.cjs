...
fix('src/components/AppProjects.tsx', [
    ...
    // Fix accessibility issue on div with click handler
    { find: /<div\s+className="p-5 rounded-2xl bg-cyan-500\/5 hover:bg-cyan-500\/10 border-2 border-dashed border-cyan-500\/20 hover:border-cyan-400\/50 transition-all duration-300 flex flex-col items-center justify-center min-h-\[160px\] shadow-\[0_0_15px_rgba\(6,182,212,0\.05\)\] hover:shadow-\[0_0_25px_rgba\(6,182,212,0\.25\)\] group cursor-pointer"\s+onClick=\{\(\) => setAddingProject\(true\)\}\s*>/g, replace: '<button\n              type="button"\n              className="p-5 rounded-2xl bg-cyan-500/5 hover:bg-cyan-500/10 border-2 border-dashed border-cyan-500/20 hover:border-cyan-400/50 transition-all duration-300 flex flex-col items-center justify-center min-h-[160px] shadow-[0_0_15px_rgba(6,182,212,0.05)] hover:shadow-[0_0_25px_rgba(6,182,212,0.25)] group cursor-pointer"\n              onClick={() => setAddingProject(true)}\n            >' },
    { find: /<\/p>\n            <\/div>/g, replace: '</p>\n            </button>' }
]);
