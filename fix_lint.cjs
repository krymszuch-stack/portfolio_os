...
fix('src/components/AppProjects.tsx', [
    ...
    { find: /<div className="p-5 rounded-2xl bg-cyan-500\/5 hover:bg-cyan-500\/10 border-2 border-dashed border-cyan-500\/20 hover:border-cyan-400\/50 transition-all duration-300 flex flex-col items-center justify-center min-h-\[160px\] shadow-\[0_0_15px_rgba\(6,182,212,0\.05\)\] hover:shadow-\[0_0_25px_rgba\(6,182,212,0\.25\)\] group cursor-pointer" onClick=\{\(\) => setAddingProject\(true\)\}>/g, replace: '<div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === \'Enter\' || e.key === \' \') setAddingProject(true); }} className="p-5 rounded-2xl bg-cyan-500/5 hover:bg-cyan-500/10 border-2 border-dashed border-cyan-500/20 hover:border-cyan-400/50 transition-all duration-300 flex flex-col items-center justify-center min-h-[160px] shadow-[0_0_15px_rgba(6,182,212,0.05)] hover:shadow-[0_0_25px_rgba(6,182,212,0.25)] group cursor-pointer" onClick={() => setAddingProject(true)}>' },
]);
